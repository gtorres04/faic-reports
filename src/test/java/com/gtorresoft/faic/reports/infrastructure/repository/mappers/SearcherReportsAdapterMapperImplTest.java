...
class SearcherReportsAdapterMapperImplTest {
  ...
  @InjectMocks SearcherReportsAdapterMapperImpl searcherReportsAdapterMapper;

  @ParameterizedTest
  @CsvSource({"'',ABC,'$2.502.134,50','',ABC,2502134.50", "'',ABC,$2.502.134,'',ABC,2502134"})
  void listObjectsToReport(
      String idTest,
      String fullNameTest,
      String previousCapitalTest,
      String id,
      String fullName,
    ...
    IntStream.range(5, 17)
        .sequential()
        .forEach(value -> list.add(value, String.format("%s%s", "$", value * 10000)));
    val result = searcherReportsAdapterMapper.listObjectsToReport(list);

    assertThat(result.id()).isEqualTo(id);
    assertThat(result.fullName()).isEqualTo(fullName);
    assertThat(result.previousCapital()).isEqualTo(previousCapital);
    Arrays.asList(Month.values())
        .forEach(
            month ->
                assertThat(result.quotaByMonth().get(month))
                    .isEqualTo(Double.valueOf((month.ordinal() + 5) * 10000)));
  }
}
